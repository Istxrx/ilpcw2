...
public class PathOld {
    ...
    public PathOld(ArrayList<Point> points, ArrayList<Integer> moveDirections, double length) {
        this.points = points;
        this.moveDirections = moveDirections;
        this.length = length;
    }

    public PathOld(Point startPoint) {
    ...
    public ArrayList<PathOld> findContinuations(double moveLength, ArrayList<Integer> directions,
            ArrayList<Polygon> obstacles) {

        var paths = new ArrayList<PathOld>();
        ...
        for (Integer direction : directions) {
            ...
            if (!Utils2D.lineIntersectPolygons(start, end, obstacles)) {
                ...
                paths.add(new PathOld(newPoints, newMoveDirections, this.length + moveLength));
            }
        }
        return paths;
    }
    ...
    public static PathOld chooseBestPath(ArrayList<PathOld> paths, Point target) {
        var bestPath = paths.get(0);
        var bestHeuristicValue = bestPath.weightedHeuristicValue(target, 1.5);

        for (int i = 1; i < paths.size(); i++) {
            var currentPath = paths.get(i);
            var currentHeuristicValue = currentPath.weightedHeuristicValue(target, 1.5);
    ...
    public static PathOld findPathToPoint(Point start, Point target, double range, double moveLength,
            ArrayList<Integer> directions, ArrayList<Polygon> obstacles) {

        var startingPath = new PathOld(start);
